...
export class ResultsPanel {
  ...
  public static createOrShow(
    ...
    buggyCode: string,
    generatedFix: GenerateFixResponse | null
  ): void {
  ...
  private async _applyFix(code: string): Promise<void> {
  ...
  private _buildHtml(
    ...
    const cards = response.results.map((r, index) => {
      ...
      const fullFixEscaped = escapeHtml(
        r.fixed_code.length > 800
...
function runLcsDiff(buggy: string, fixed: string): DiffLine[] {
...
function buildInlineDiff(diffLines: DiffLine[]): string {
  let buggyLineNum = 1;
  let fixedLineNum = 1;
